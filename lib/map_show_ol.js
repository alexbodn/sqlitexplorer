...
class DisplayOpenLayers extends DisplayDriver {
	static resource_urls = {
		...
		js: [
			...
			"./node_modules/@nieuwlandgeo/sldreader/dist/sldreader.js"
		]
	};
	...
	calcExtent(layers) {
		...
		//console.log('calc extent', extent, ol.extent.isEmpty(extent));
		return extent;
	}
	...
	show_map(viewOptions) {
		...
		if (!viewOptions) {
			...
			//	console.error('fit', extent, map.getSize());
			if (ol.extent.isEmpty(extent)) {
				//extent = [-180, -90, 180, 90];
				let extreme = Math.PI * 6378137;
				extent = [
					-extreme, -extreme,
					extreme, extreme,
				];
