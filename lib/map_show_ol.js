...
import { DisplayDriver } from "./map_show.js";

//import {Map, View} from '../node_modules/ol/dist/ol.js';
//import {Map, extent, proj, source, layer, format, color, expr, render, style, View, control, coordinate} from '../node_modules/ol/dist/ol.js';

///import * as LayerSwitcher from '../node_modules/ol-layerswitcher/dist/ol-layerswitcher.js';
///import "./sldreader.js";
//console.log(Map);
...
class DisplayOpenLayers extends DisplayDriver {
	...
	static async addJSON(layer, json) {
		...
		if (
			crsSection?.type === "name" ||
			(crsSection?.properties?.name && !crsSection.type)
		) {
		...
		for (let feature of features) {
			...
			if (flatstyle) {
				...
				let style = ol.render.canvas.style.buildStyle(
					flatstyle,
					parsingContext
				)();
		...
		map.on("singleclick", evt => {
			...
			map.forEachFeatureAtPixel(
				...
				(feature, layer) => {
					...
					if (!feature) {
						feature =
							layer.getClosestFeatureToCoordinate(coordinates);
					}
					if (feature) {
						...
						let layerCoordinates = ol.proj.transform(
							coordinates,
	...
	static makeTiledLayer(name, id, { min_zoom, max_zoom, bounds, fetchTile }) {
		...
		let extent = ol.proj.transformExtent(extentw, "EPSG:4326", "EPSG:3857");
		...
		let tileSource = new ol.source.XYZ({
			...
			tileLoadFunction: async function (tile, src) {
				...
				let timeLabel = `tile_${[1, 2, 0]
					.map(c => coords[c])
					.join(",")}`;
				console.time(timeLabel);
				fetchTile(coords[1], coords[2], coords[0])
					.then(url => {
						if (url) {
...
export { DisplayOpenLayers as DisplayDriver };
