...
//map show wkt from query textarea
...
const conditionalSchema = {
	sqlite_schema: `
		...
		table_info.tbl_name ||
		case
		when {database}.sqlite_schema.type in ('index', 'trigger')
			then ' (' || table_info.type || ')'
		else ' (' || {database}.sqlite_schema.type || ')'
		end ||
			' [button\tbrowse\tbrowseTable\t' ||
			{database}.sqlite_schema.tbl_name || ']'
		as entity,
		case
		when {database}.sqlite_schema.type='index'
			then {database}.sqlite_schema.type || ' ' || {database}.sqlite_schema.name
		when {database}.sqlite_schema.type='trigger'
			then 'triggers'
		else 'columns'
		end as [group],
		case
		...
		when {database}.sqlite_schema.type='trigger'
			then {database}.sqlite_schema.name
		...
		FROM {database}.sqlite_schema
		inner join {database}.sqlite_schema as table_info
			on table_info.tbl_name={database}.sqlite_schema.tbl_name
				and table_info.type in ('table', 'view')
		left outer JOIN pragma_table_info({database}.sqlite_schema.name) table_fields
		left outer JOIN pragma_index_info({database}.sqlite_schema.name) index_fields
		WHERE {database}.sqlite_schema.name NOT LIKE 'sqlite_%'
			and {database}.sqlite_schema.name not like 'SpatialIndex'
			and {database}.sqlite_schema.name not like 'ElementaryGeometries'
		order by 
		table_info.tbl_name, 
		table_info.type desc,
		[group],
	...
	gpkg_contents: `
		...
		case
			when data_type='attributes' or :withMap=0
				or data_type='features' and :isSpatial=0
		...
		as column
		FROM {database}.gpkg_contents as gpkg_contents
		left outer join {database}.gpkg_geometry_columns as gpkg_geometry_columns
			on {database}.gpkg_contents.table_name={database}.gpkg_geometry_columns.table_name
	...
	vector_layers: `
		...
		select
		'spatialite/+', 'vector_layers/+', layer_type || '/+',
		table_name ||
			case
			when :withMap=1 and :isSpatial=1
				then ' [button\tmap\tmapTable\tfeatures\t' ||
				table_name || '\t' ||
				geometry_column || ']'
		...
		' coord_dimension:' ||
		cast (coord_dimension as TEXT) ||
		' srid:' ||
		cast (srid as TEXT) ||
		case when spatial_index_enabled=1
			then ' spatial_index_enabled'
			else ''
		end as column
		from {database}.vector_layers as vector_layers
		left outer join geometry_types as geometry_types
			on geometry_types.number=geometry_type
		`,
	views_geometry_columns: `
		select 
		'spatialite/+', 'geometries/+', 'views/+',
		view_name as [view],
		view_geometry as geometry
		from {database}.views_geometry_columns`,
	virts_geometry_columns: `
		select 
		'spatialite/+', 'geometries/+', 'virts/+',
		virt_name as virt,
		virt_geometry as geometry
		from {database}.virts_geometry_columns`,
	networks: `
		select 
		'spatialite/+', 'networks/+',
		network_name,
		null, null
		from {database}.networks`,
	topologies: `
		select 
		'spatialite/+', 'topologies/+',
		topology_name,
		null, null
		from {database}.topologies`,
	raster_coverages: `
		select 
		'spatialite/+', 'raster_coverages/+',
		coverage_name,
		null, null
		from {database}.raster_coverages`,
	vector_coverages: `
		select 
		'spatialite/+', 'vector_coverages/+',
		coverage_name,
		null, null
		from {database}.vector_coverages`,
	stored_procedures: `
		select 
		'spatialite/+', 'stored_procedures/+',
		name,
		null, null
		from {database}.stored_procedures`,
	stored_variables: `
		select 
		'spatialite/+', 'stored_variables/+',
		name,
		null, null
		from {database}.stored_variables`,
};
...
class SQLiteXplorer {
	...
	constructor(container, {id, build_map, mapParams, withProj4JS}={}) {
	...
	createMainMenu() {
		this.createTab(
			...
			<div class="query-container">
				<table border="1">
				<tbody class="mainMenu">
				<tr>
				<td>
				<div class="documentation">
					<button onclick="window.open('https://www.gaia-gis.it/fossil/libspatialite/index', '_BLANK', '', '');">SpatiaLite Documentation</button>
				</div>
				<div class="documentation">
					<button onclick="window.open('https://github.com/jvail/spl.js', '_BLANK', '', '');">spl.js</button>
				</div>
				<div class="documentation">
					<button onclick="window.open('https://www.sqlite.org/docs.html', '_BLANK', '', '');">SQLite Documentation</button>
				</div>
				<div class="documentation">
					<button onclick="window.open('https://sqlite.org/wasm/doc/trunk/index.md', '_BLANK', '', '');">sqlite3 WebAssembly & JavaScript</button>
				</div>
				</td>
				</tr>
				</tbody>
	...
	createCopyright() {
		const notice = `
			 * Copyright &copy; 2023, 2024 Alex Bodnaru <alexbodn@gmail.com>
			 * 
			 * This file is part of SQLiteXplorer.
			 * SQLiteXplorer is free software: 
			 * you can redistribute it and/or modify it 
			 ...
			 * GNU Lesser General Public License along 
			 * with SQLiteXplorer. If not, 
			 * see  <a href="https://www.gnu.org/licenses/lgpl-3.0-standalone.html">the full license text</a>.
		`;
		...
		this.copyright_tab = tab.dataset.tabValue;
	}
	
	showCopyright() {
		let copyright_tab = this.tabFetch(this.copyright_tab);
		this.tabActivate(copyright_tab);
	}
	...
	ppQuery(button, uncomment=false) {
		...
		let jsonData = this.parseGeoJSON(query);
		if (jsonData) {
			query = JSON.stringify(jsonData, null, '\t');
		}
		else {
		...
		if (objs.length || error) {
			...
			tfoot.insertAdjacentHTML(
				...
				`<tr><td colspan="${colnames.length}">
					...
					<span style="display: ${csvDisplay};"><input type="checkbox" checked="checked" class="csv-headers" />headers</span>
	...
	dataPopup(td) {
		...
		let divMime = document.createElement('div');
		divMime.insertAdjacentHTML(
			'beforeend',
	...
	csvResults(button) {
		...
		let csv = [];
		let entities = [];
		if (table.querySelector('.csv-headers').checked) {
			entities.push(table.querySelector('thead'));
		}
		entities.push(table.querySelector('tbody'));
	...
	parseGeoJSON(text) {
		...
		try {
			obj = JSON.parse(text);
			if (obj && !obj.features && !obj.geometry && !obj.coordinates) {
				obj = null;
			}
		}
		catch(e) {
	...
	async featuresLayer(layerLabel, layerId, sldStyle, query, params={}) {
		...
		if (objs.length) {
			...
			for (let row of objs) {
				if (row.feature) {
					featureCRS = row.feature.crs?.properties?.name;
					...
					if (this.withProj4JS && featureCRS && featureCRS != targetCRS) {
						await this.loadProjections([featureCRS]);
						row.feature = this.featureProject(
							featureCRS, targetCRS, row.feature);
					}
				}
			}
			if (geometry) {
		...
		if (extent) {
			if (this.withProj4JS && featureCRS && featureCRS != targetCRS) {
				extent = this.featureProject(
					featureCRS, targetCRS, extent);
			}
		}
		if (extent) {
	...
	featureProject = (source, target, feature, {depth=0, projector}={}) => {
		const chunkProject = (chunk) => {
			return this.featureProject(
				source, target, chunk, {depth: depth + 1, projector});
		};
		if (Array.isArray(feature)) {
			if (
				projector &&
				feature.length > 1 &&
				typeof feature[0] === 'number' &&
				typeof feature[1] === 'number'
				) {
					return projector.forward(feature);
			}
			else {
				feature = feature.map(chunkProject);
			}
		}
		else if (feature && typeof feature === 'object') {
			for (let key in feature) {
				if (key === 'coordinates') {
					projector = proj4(source, target);
				}
				feature[key] = chunkProject(feature[key]);
			}
		}
	...
	dataExtent = (data, {depth=0, inCoordinates=false, extent}={}) => {
		if (!depth) {
		...
		if (Array.isArray(data)) {
			if (
				...
				typeof data[1] === 'number'
					...
					extent[0] = Math.min(extent[0], data[0]);
					extent[2] = Math.max(extent[2], data[0]);
					extent[1] = Math.min(extent[1], data[1]);
					extent[3] = Math.max(extent[3], data[1]);
			}
			else {
	...
	async showJSON(tabId, tabLabel, feature) {
		const defaultCRS = 'EPSG:4326';
		if (!feature.crs) {
			feature.crs = {
				properties: {name: defaultCRS},
				type: 'name',
			};
		}
		let extent = this.dataExtent(feature);
		const featureCRS = feature.crs.properties.name;
		if (this.withProj4JS && featureCRS != defaultCRS) {
			await this.loadProjections([featureCRS]);
			//beware: openlayers driver won't correctly
			//locate on map non 4326 geometries
			feature = this.featureProject(
				featureCRS, defaultCRS, feature);
			extent = this.featureProject(
				featureCRS, defaultCRS, {coordinates: extent}
			)?.coordinates;
		}
		return this.showLayer(tabId, tabLabel, [{feature}], extent);
	}
	...
	async showLayer(tabId, tabLabel, objs, extent, {sldStyle}={}) {
		...
		await this.loadProjections(projections);
		this.map.addLayer(layer);
		this.layersOnMap[tabId] = tabLabel;
		this.showMap();
	}
	
	async loadProjections(codes) {
		if (!this.withProj4JS || !codes) {
			return;
		}
		let projections = {};
		...
		for (let code of codes) {
			...
			.catch(error => {
				console.log('error loading projectio(waaans', error);
			});
		}
		proj4.defs(Object.entries(projections));
	}
	...
	showMap = (viewOptions) => {
		let map_tab = this.tabSearch('map_tab');
		if (map_tab) {
	...
	async buildConditionalSchema(database='main') {
		let entities = await this.performQuery(`
			SELECT json(json_group_object(name, type))
			FROM ${database}.sqlite_schema
			WHERE type IN ('table', 'view')
		`).first;
		entities.sqlite_schema = 'schema';
		let available = Object.entries(conditionalSchema)
			...
			.map(([entity, query]) => {
				let {rows} = this.performQuery(
					...
					{
						withMap: this.withMap ? 1 : 0,
						isSpatial: this.isSpatial ? 1 : 0,
					}
				);
				return rows;
			});
	...
	buildDatabaseTree(database) {
		return this.buildConditionalSchema(database)
		.then(groups => {
			let tree = [];
			for (let group of groups) {
				if (group.status == "fulfilled") {
					let subTree = gridTree(group.value);
					if (subTree) {
						tree.push(...subTree);
					}
				}
	...
	async setSchema(tab) {
		let dbNames = await this.performQuery(`
			select name
			from PRAGMA_database_list`).flat;
		Promise.all(
			dbNames.map(database =>
				this.buildDatabaseTree(database)
			)
		)
		.then(trees => {
			let html = htmlTree(trees);
	...
	createMapTab() {
		...
		this.map = new this.build_map(map_container, ...this.mapParams);
	}
	...
	createTab(label, content, {
		...
		tabs.insertAdjacentHTML(
			'beforeend',
			`<span data-tab-value=".${_class}" data-search-tag="${searchTag || null}">
				${label}${closeX}
			</span>`
		);
	...
	addQueryTab = (label, query='', params={}) => {
			...
			<div class="query-with-controls" style="width: 100%;">
				...
				<div class="query-controls" style="width: 100%;">
					...
					<div class="query-control" style="width: 28%;">
						...
						<button class="preproc-query">preproc</button>
		...
		queryElem.value = query;
		let help = `
			-- write geoJSON to show on map,
			-- or sql queries to run or show.
		...
		tabInfo.querySelector('button.preproc-query')
	...
	tabSearch(searchTag) {
		let tabInfo = this.mainForm.querySelector(
			`span[data-search-tag="${searchTag}"]`);
		return tabInfo;
	}
	
	//return the tab content
	tabInfo = tab => this.mainForm
		.querySelector(tab?.dataset.tabValue);
};
