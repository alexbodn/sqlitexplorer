...
export class SelectHitTolerance {
	formCode = `
		<form>
			<label>
...
export class DisplayDriver {
	...
	static resource_url(url, currentScript, prefix) {
	...
	static resources(currentScript, prefix) {
		return Promise.all([
			this.js_resources(currentScript, prefix),
			this.css_resources(currentScript, prefix),
		]);
	}
	
	static js_resources(currentScript, prefix) {
		let jsAll = Promise.resolve();
		for (let js of this.resource_urls.js) {
			jsAll = jsAll.then(() => {
				return this.jsLoad(this.resource_url(js, currentScript, prefix));
			});
		}
		return jsAll;
	}
	
	static css_resources(currentScript, prefix) {
		let cssAll = Promise.resolve();
		for (let css of this.resource_urls.css) {
			cssAll = cssAll.then(() => {
				return this.cssLoad(this.resource_url(css, currentScript, prefix));
			});
		}
		return cssAll;
	}
